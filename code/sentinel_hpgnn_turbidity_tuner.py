...
data = pd.read_excel(file_path, 'turb_')  # Load Turbidity data

# Reflectance Bands
reflectance_bands = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8']
features = reflectance_bands
targets = ['Turbidity']  # Target is Turbidity
...
def pinn_loss(y_true, y_pred, X_batch, w1, w2, w3, w4, w5, w6):
    ...
    a = tf.constant(177.4322, dtype=tf.float32)
    b = tf.constant(0.5456, dtype=tf.float32)
    
    Turbidity_chen = a * (R_b4**b)
    
    physics_loss_2 = tf.reduce_mean(tf.square(y_pred - Turbidity_chen))
...
def build_model(hp):
    ...
    # Output layer
    output_layer = Dense(1, name='Turbidity')(x)
...
tuner = run_tuner(X_train, y_train['Turbidity'], X_test, y_test['Turbidity'],
                  directory='F:/Paper/Eco_Inf/tuner', project_name='Turbidity_sentinel_optimization', seed=42)
...
history = best_model.fit(X_train, y_train['Turbidity'], 
                         validation_data=(X_test, y_test['Turbidity']), 
...
save_model(best_model, "F:/Paper/Eco_Inf/models/Turbidity_HPGNN_Model.h5")

with open("F:/Paper/Eco_Inf/models/Turbidity_HPGNN_Model_Hyperparams.json", "w") as f:
    json.dump(hyperparams, f)

# Load New Data for Testing
new_data_path = "F:/Paper/Eco_Inf/data/matchup_data_test.xlsx"
new_data = pd.read_excel(new_data_path, 'turb_')
...
mae_train = mean_absolute_error(y_train['Turbidity'], y_pred_train)
r2_train = r2_score(y_train['Turbidity'], y_pred_train)
rmse_train = np.sqrt(mean_squared_error(y_train['Turbidity'], y_pred_train))

mae_test = mean_absolute_error(y_test['Turbidity'], y_pred_test)
r2_test = r2_score(y_test['Turbidity'], y_pred_test)
rmse_test = np.sqrt(mean_squared_error(y_test['Turbidity'], y_pred_test))

mae_test_ = mean_absolute_error(new_data['Turbidity'], y_pred_test_)
r2_test_ = r2_score(new_data['Turbidity'], y_pred_test_)
rmse_test_ = np.sqrt(mean_squared_error(new_data['Turbidity'], y_pred_test_))
