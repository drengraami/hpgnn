...
data = pd.read_excel(file_path, 'tss_')  # Load TSS data

# 
# Reflectance Bands
reflectance_bands = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8']
features = reflectance_bands
targets = ['TSS']  # Target is TSS
...
# Define Custom PINN Loss Function with Adaptive Weights
class AdaptiveWeightsCallback(Callback):
    def __init__(self, w1, w2, w3, w4, w5, w6):
        ...
        self.w4 = w4  # Weight for cross-validation loss
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.w4 = min(0.1, self.w4 + 0.001)  # Gradually increase cross-validation weight
...
def pinn_loss(y_true, y_pred, X_batch, w1, w2, w3, w4, w5, w6):
    ...
    f = tf.constant(6.87377714, dtype=tf.float32)
    g = tf.constant(2.57043087e+02, dtype=tf.float32)
    h = tf.constant(9.04560420e+05, dtype=tf.float32)

    tss_nechad = f + (g * np.pi * R_b4) / (1 - (np.pi * R_b4) / h)
    
    physics_loss_2 = tf.reduce_mean(tf.square(y_pred - tss_nechad))
...
def build_model(hp):
    ...
    # Output layer
    output_layer = Dense(1, name='TSS')(x)
...
tuner = run_tuner(X_train, y_train['TSS'], X_test, y_test['TSS'],
                  directory='F:/Paper/Eco_Inf/tuner', project_name='TSS_sentinel_optimization', seed=42)
...
history = best_model.fit(X_train, y_train['TSS'], 
                         validation_data=(X_test, y_test['TSS']), 
...
save_model(best_model, "F:/Paper/Eco_Inf/models/TSS_HPGNN_Model.h5")

with open("F:/Paper/Eco_Inf/models/TSS_HPGNN_Model_Hyperparams.json", "w") as f:
    json.dump(hyperparams, f)

# Load New Data for Testing
new_data_path = "F:/Paper/Eco_Inf/data/matchup_data_test.xlsx"
new_data = pd.read_excel(new_data_path, 'tss_')
...
mae_train = mean_absolute_error(y_train['TSS'], y_pred_train)
r2_train = r2_score(y_train['TSS'], y_pred_train)
rmse_train = np.sqrt(mean_squared_error(y_train['TSS'], y_pred_train))

mae_test = mean_absolute_error(y_test['TSS'], y_pred_test)
r2_test = r2_score(y_test['TSS'], y_pred_test)
rmse_test = np.sqrt(mean_squared_error(y_test['TSS'], y_pred_test))

mae_test_ = mean_absolute_error(new_data['TSS'], y_pred_test_)
r2_test_ = r2_score(new_data['TSS'], y_pred_test_)
rmse_test_ = np.sqrt(mean_squared_error(new_data['TSS'], y_pred_test_))
