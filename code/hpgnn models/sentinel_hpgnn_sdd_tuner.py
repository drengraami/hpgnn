...
data = pd.read_excel(file_path, 'sdd_')  # Load Secchi_depth data

# Reflectance Bands
reflectance_bands = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8']
features = reflectance_bands
targets = ['Secchi_depth']  # Target is Secchi_depth
...
class AdaptiveWeightsCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        self.w4 = min(0.1, self.w4 + 0.001)  # Gradually increase variancen weight
...
def pinn_loss(y_true, y_pred, X_batch, w1, w2, w3, w4, w5, w6):
    ...
    # Empirical bio-optical model
    R_b4 = tf.cast(X_batch[:, 3], dtype=tf.float32)  
    R_b2 = tf.cast(X_batch[:, 1], dtype=tf.float32)  

    # Bio-optical model coefficients
    a = tf.constant(0.9250, dtype=tf.float32)
    b = tf.constant(0.8294, dtype=tf.float32)
    c = tf.constant(-54.5304, dtype=tf.float32)

    Secchi_depth_olmanson = a + b* tf.math.divide_no_nan(R_b2, R_b4) + c * R_b2

    physics_loss_2 = tf.reduce_mean(tf.square(y_pred - Secchi_depth_olmanson))
...
def build_model(hp):
    ...
    # Output layer
    output_layer = Dense(1, name='Secchi_depth')(x)
...
tuner = run_tuner(X_train, y_train['Secchi_depth'], X_test, y_test['Secchi_depth'],
                  directory='F:/Paper/Eco_Inf/tuner', project_name='Secchi_depth_sentinel_optimization', seed=42)
...
history = best_model.fit(X_train, y_train['Secchi_depth'], 
                         validation_data=(X_test, y_test['Secchi_depth']), 
...
save_model(best_model, "F:/Paper/Eco_Inf/models/Secchi_depth_HPGNN_Model.h5")

with open("F:/Paper/Eco_Inf/models/Secchi_depth_HPGNN_Model_Hyperparams.json", "w") as f:
    json.dump(hyperparams, f)

# Load New Data for Testing
new_data_path = "F:/Paper/Eco_Inf/data/matchup_data_test.xlsx"
new_data = pd.read_excel(new_data_path, 'sdd_')
...
mae_train = mean_absolute_error(y_train['Secchi_depth'], y_pred_train)
r2_train = r2_score(y_train['Secchi_depth'], y_pred_train)
rmse_train = np.sqrt(mean_squared_error(y_train['Secchi_depth'], y_pred_train))

mae_test = mean_absolute_error(y_test['Secchi_depth'], y_pred_test)
r2_test = r2_score(y_test['Secchi_depth'], y_pred_test)
rmse_test = np.sqrt(mean_squared_error(y_test['Secchi_depth'], y_pred_test))

mae_test_ = mean_absolute_error(new_data['Secchi_depth'], y_pred_test_)
r2_test_ = r2_score(new_data['Secchi_depth'], y_pred_test_)
rmse_test_ = np.sqrt(mean_squared_error(new_data['Secchi_depth'], y_pred_test_))

print(f"\nTrain R²: {r2_train:.4f}, RMSE: {rmse_train:.4f}, MAE: {mae_train:.4f}")
print(f"Test R²: {r2_test:.4f}, RMSE: {rmse_test:.4f}, MAE: {mae_test:.4f}")
print(f"New Data R²: {r2_test_:.4f}, RMSE: {rmse_test_:.4f}, MAE: {mae_test_:.4f}")
