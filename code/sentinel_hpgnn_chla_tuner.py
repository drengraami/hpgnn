...
data = pd.read_excel(file_path, 'chla_')  # Load Chla data

# Reflectance Bands
reflectance_bands = ['B1', 'B2', 'B3', 'B4', 'B5', 'B6', 'B7', 'B8']
features = reflectance_bands
targets = ['Chla']  # Target is Chla
...
def pinn_loss(y_true, y_pred, X_batch, w1, w2, w3, w4, w5, w6):    
    ...
    R_b5 = tf.cast(X_batch[:, 4], dtype=tf.float32)  
...
def build_model(hp):
    ...
    # Output layer
    output_layer = Dense(1, name='Chla')(x)
...
tuner = run_tuner(X_train, y_train['Chla'], X_test, y_test['Chla'],
                  directory='F:/Paper/Eco_Inf/tuner', project_name='Chla_sentinel_optimization', seed=42)
...
history = best_model.fit(X_train, y_train['Chla'], 
                         validation_data=(X_test, y_test['Chla']), 
...
save_model(best_model, "F:/Paper/Eco_Inf/models/Chla_HPGNN_Model.h5")

with open("F:/Paper/Eco_Inf/models/Chla_HPGNN_Model_Hyperparams.json", "w") as f:
    json.dump(hyperparams, f)

# Load New Data for Testing
new_data_path = "F:/Paper/Eco_Inf/data/matchup_data_test.xlsx"
new_data = pd.read_excel(new_data_path, 'chla_')
...
mae_train = mean_absolute_error(y_train['Chla'], y_pred_train)
r2_train = r2_score(y_train['Chla'], y_pred_train)
rmse_train = np.sqrt(mean_squared_error(y_train['Chla'], y_pred_train))

mae_test = mean_absolute_error(y_test['Chla'], y_pred_test)
r2_test = r2_score(y_test['Chla'], y_pred_test)
rmse_test = np.sqrt(mean_squared_error(y_test['Chla'], y_pred_test))

mae_test_ = mean_absolute_error(new_data['Chla'], y_pred_test_)
r2_test_ = r2_score(new_data['Chla'], y_pred_test_)
rmse_test_ = np.sqrt(mean_squared_error(new_data['Chla'], y_pred_test_))
