...
def run_tuner(X_train, y_train, X_test, y_test, directory, project_name, seed=1):
    ...
    tuner.search(X_train, y_train,
                 ...
                 epochs=1000,
                 batch_size=64,
                 callbacks=[early_stopping, ReduceLROnPlateau(monitor='val_mae', factor=0.5, patience=200, min_lr=1e-6, verbose=1)])

    return tuner

# Run Hyperparameter Tuning
tuner = run_tuner(X_train, y_train['Chla'], X_test, y_test['Chla'],
                  directory='F:/Paper/Eco_Inf/tuner', 
                  project_name='Chla_sentinel_optimization', seed=42)
...
def run_ablation_study(X_train, y_train, X_test, y_test, X_new, y_new, best_hp, features):
    ...
    for config_name, weights in configurations:
        ...
        model_save_path = os.path.join(MODEL_SAVE_DIR, f"model_{config_name.lower()}.h5")
...
# Run Ablation Study with New Test Data
